...
namespace CabNotificationService.Handlers.Notification
{
    public partial class NotificationHandler : IQueryHandler<GetNotificationQuery, PagingResponse<NotificationResponse>>
    {
        public async Task<PagingResponse<NotificationResponse>> Handle(GetNotificationQuery request, CancellationToken cancellationToken)
        {
            ...
            Expression<Func<Models.Entities.NotificationUserIdMaterializedView, bool>> predicate;

            if (request.Type == NotificationConstant.System)
            {
                predicate = x => x.Type == NotificationConstant.System && x.UserId == request.UserId;
            }
            else
            {
            ...
            var (data, pagingState) = await notificationUserIdRepository
                .GetListPagingAsync(predicate, request.PageSize, pagingStateByte);
            ...
            response.Data = notificationsResponse;
            ...
            response.HasMore = response.Total - (response.PageNumber * response.PageSize) > 0;
